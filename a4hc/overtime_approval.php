...
if(empty($_SESSION['login_user']) or $auth == "regular") {
	if ($url_overtime_id == "") {
		...
		echo "<meta http-equiv=\"refresh\" content=\"0; url=./login?path=overtimeapproval&hash=overtime_request_view_&id=$url_overtime_id\">";		
	}
...
<div class="splash"> <div class="color-line"></div><div class="splash-title"><h1>Overtime Request Page</h1><p>Loading...</p><div class="spinner"> <div class="rect1"></div> <div class="rect2"></div> <div class="rect3"></div> <div class="rect4"></div> <div class="rect5"></div> </div> </div> </div>
<!--[if lt IE 7]>
    ...
    <div class="normalheader transition animated fadeIn small-header">
        <div class="hpanel">
            <div class="panel-body">
                ...
                <small>You can approve employee's overtime requests here!</small>
				</div>
            </div>
        </div>
    </div>
...
<?
if ($auth == "admin") {
	$query_overtime_result = mysqli_query($db, "SELECT * FROM overtime_request");
}else {
	$query_overtime_result = mysqli_query($db, "SELECT * FROM overtime_request WHERE notifier='$user'");
}
        ...
        <div class="col-lg-12">
            ...
            <div class="tab-content">
                ...
                <tbody>
								<?								
								if ($auth == "admin") {
									$query_overtime_result = mysqli_query($db, "SELECT * FROM overtime_request WHERE application_status='Submitted' ORDER BY id asc");
								}else {
									$query_overtime_result = mysqli_query($db, "SELECT * FROM overtime_request WHERE notifier='$user' AND application_status='Submitted' ORDER BY id asc");
								}
								...
								<div id="view_<?echo $modal_detail_id;?>" class="remodal-bg">
									<div class="remodal" data-remodal-id="<?echo $modal_detail_id;?>" data-remodal-options="hashTracking: true, closeOnOutsideClick: false">
										...
										<!--	<div class="row">
											...
											<textarea id="overtime_approval_<?echo $overtime_request_id?>" class="form-control field_value_<?echo $overtime_request_id?>" rows="5" placeholder="Write a comment here..."></textarea>
										...
										<div class="modal-footer">
											...
											<div class="pull-left">
												<button type="button" class="btn btn-success action_button" id="approve_<?echo $overtime_request_id?>">Approve</button>
												<button type="button" class="btn btn-danger action_button" id="reject_<?echo $overtime_request_id?>">Reject</button>
											</div>										
										</div>
									</div><!-- tab-content -->
								</div> <!-- /form --> 								
                ...
                <tbody>
								<?
								if ($auth == "admin") {
									$query_overtime_result = mysqli_query($db, "SELECT * FROM overtime_request WHERE application_status='Approved' ORDER BY id asc");
								}else {
									$query_overtime_result = mysqli_query($db, "SELECT * FROM overtime_request WHERE notifier='$user' AND application_status='Approved' ORDER BY id asc");
								}
									while($overtime_array = mysqli_fetch_assoc($query_overtime_result)) {
										// $date_from = $overtime_array['date_from'];
										// $date_to = $overtime_array['date_to'];
								...
								?>
						<tr>
							<td><strong><?echo "$time_stamp_created_format";?> by <?echo $requested_user_name?></strong></strong></td>
							<?
                ...
                <tbody>
								<?
								if ($auth == "admin") {
									$query_overtime_result = mysqli_query($db, "SELECT * FROM overtime_request WHERE application_status='Rejected' ORDER BY id asc");
								}else {
									$query_overtime_result = mysqli_query($db, "SELECT * FROM overtime_request WHERE notifier='$user' AND application_status='Rejected' ORDER BY id asc");
								}
									while($overtime_array = mysqli_fetch_assoc($query_overtime_result)) {
										// $date_from = $overtime_array['date_from'];
										// $date_to = $overtime_array['date_to'];
...
<!-- make request modal -->
	<div id="overtime_remodal" class="remodal-bg">
		<div class="remodal" data-remodal-id="overtime_request" data-remodal-options="hashTracking: false, closeOnOutsideClick: false">
				...
				<div class="modal-header">
					<h4 class="modal-title">Make a Overtime Request</h4>
				</div>
