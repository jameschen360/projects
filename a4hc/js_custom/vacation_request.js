$(document).ready(function(){
	...
	$('#submit_vacation').click(function(){
				
		var inst = $('[data-remodal-id=vacation_request]').remodal();//obtain status of remodal
		...
		// if (date_check1!=null && date_check2!=null) {
			// $("#date_to, #date_from").removeClass("vacation-css-error");
			// $("#date_to, #date_from").addClass("vacation-css-success");
			...
			if ($.trim(reason_check_content).length > 0) {// check if reason content is empty
				...
				$("#submit_vacation").prop("disabled", true);
				$(".close_main").prop("disabled", true);
				$("#submit_vacation").html("<span class='glyphicon glyphicon-refresh glyphicon-refresh-animate'></span> Processing...");
				swal({
				  ...
				  confirmButtonText: 'Yes!'
					...
					$("#submit_vacation").html("<span class='glyphicon glyphicon-refresh glyphicon-refresh-animate'></span> Processing...");	
					$("#submit_vacation").prop("disabled", true);
					$("#vacation_request_close").prop("disabled", true);
					...
					$.ajax({
						type: "POST",
						url: "./services/vacation_request.php",
						data: formData,
						cache: false,
						contentType: false,
						processData: false,					
							...
							if (data === "Files must be less than 5MB." || data === "Only PDF files are allowed.") {
								swal({
								  ...
								  confirmButtonText: 'Try Again'
									...
									$("#submit_vacation").prop("disabled", false);
									$("#submit_vacation").html("Submit");
									$("#vacation_request_close").prop("disabled", false);
								})
							}else {
								$.LoadingOverlay("hide", true);
								$("#submit_vacation").html("Submit");
								...
								swal({
								  ...
								  confirmButtonText: 'Okie!'
									...
									$("#submit_vacation").prop("disabled", true);
									inst.close();
									$("#submit_vacation").prop("disabled", false);//enable submit button
									// $('#date_from').val('');//clear input
									// $('#date_to').val('');//clear input
									$('#vacation_file').val('');//clear input
		...
		// 	$("#date_to, #date_from").removeClass("vacation-css-success");
		// 	$("#date_to, #date_from").addClass("vacation-css-error");	
		// }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	$('.button_delete').click(function(){
		...
		var vacation_id_array = vacation_id.split('_');

		vacation_id = vacation_id_array[1];		
		...
		var vacation_id = $(this).attr('id').split('_');
		var username = $('#hidden_username').val();
		var info = 'delete_id='+vacation_id[1]+'&username='+username;	
	...
	$('.action_button').click(function(){
		...
		var username = $('#hidden_username').val();
		var comment = $('#vacation_approval_'+id).val();
					...
					success: function(data){
						...
						if (data === "no") {
							...
							swal({
							  ...
							  allowEscapeKey: false,
							  allowEnterKey: false,			
							  title: 'Time off Request has been '+purpose_msg2,
							  html: 'Redirecting <i class="glyphicon glyphicon-repeat gly-spin"></i>',
							  type: symbol,
							  timer: 2000,
							  showLoaderOnConfirm: true,
