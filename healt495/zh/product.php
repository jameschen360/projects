...
<?
  ...
  $catagory_desc_zh = $cata_sql['zh_desc'];
  $min_price = $_GET['min'];
  $max_price = $_GET['max'];
  $MAX_LENGTH = 100;
 ...
 $check_row = mysqli_num_rows(mysqli_query($db, "SELECT * FROM catagory WHERE subname LIKE '%$product_catagory%'"));
  
 $zh_catagory_name = mysqli_fetch_assoc(mysqli_query($db, "SELECT * FROM catagory WHERE subname LIKE '%$product_catagory%'"));
 if ($check_row != 0){
		$zh_catagory_name = $zh_catagory_name['zh_name'];
 }else {

	$zh_catagory_name = mysqli_fetch_assoc(mysqli_query($db, "SELECT * FROM catagory WHERE id='$catagory_name_id'")); 
	$zh_catagory_name = $zh_catagory_name['zh_name']; 
 }
			...
			<div class="breadcrumb-container">
				<div class="container">
					<ol class="breadcrumb">
						...
						<li class="active"><?echo $zh_catagory_name?></li>
					</ol>
				</div>
			</div>
			<!-- breadcrumb end -->
					...
					<div class="row">
						...
						<aside id="floating" class="col-md-3 hidden-xs hidden-sm">
							<div class="sidebar">
									...
									<nav>
														<?
															while ($i < $row_count_catagory) {
																if ($sub[$i] == "0") {?>
																	<a href="product.php?catagory=<?echo strtolower($link_name[$i]);?>&id=<?echo $id[$i]?>"><?echo $name[$i]?></a>																
																<?	$i++;
																} else {?>
																	<div id="acr">
																	...
																	<div class="collapse in catagory_acr" id="<?echo preg_replace('/\s+/', '', $name[$i])?>">
																		...
																		<?
																			for ($j = 0; $j < $sub[$i] ; $j++) {?>
																				...
																				<?$sub_catagory_link = explode(",",$link_subname[$i]);?>
																				<li><a href="product.php?catagory=<?echo strtolower($sub_catagory_link[$j]);?>&id=<?echo $id[$i]?>"><?echo $sub_catagory[$j]?></a></li>
																			<?}
																			?>
																		</ul>
																	</div>																	
								...
								<?
									if ($cata_sql_row != 0) { ?>
								<div class="block clearfix">
									<h3 class="title"><?echo $zh_catagory_name;?></h3>
									<div class="separator-2"></div>
									<p class="margin-clear"><?echo $catagory_desc_zh;?></p>
								</div>										
								<?	}
								?>
								<div class="block clearfix">
									<form role="search" action="search.php" method="GET">
										<div class="form-group has-feedback">
											<input type="text" name="keyword" class="form-control" placeholder="搜索" required autocomplete="off">
						...
						<div class="main col-md-9">
							...
							<ul class="nav nav-pills" role="tablist">
								<li class="active"><a href="#pill-1" role="tab" data-toggle="tab" title="Latest Arrivals"><i class="icon-star"></i> <?echo $zh_catagory_name?></a></li>
							</ul>
							<!-- Tab panes -->
									...
									<div class="row masonry-grid-fitrows grid-space-10">
										...
										if($row_count == 0) {
											...
											while($row = mysqli_fetch_assoc($product_fetch)) { 
												$product_id = $row['id'];
												$product_name = $row['zh_name'];
												$product_desc= $row['zh_detail'];
												$price = $row['price'];
												$picture = $row['picture'];
												$stock = $row['instock'];
												...
												<div class="listing-item white-bg bordered mb-20">
													<div class="overlay-container">
														...
														<div class="overlay-to-top links">
															<span class="small">
																<a href="product-detail.php?catagory=<?echo $product_catagory?>&id=<?echo $product_id;?>" class="btn-sm-link"><i class="icon-link pr-5"></i>更多产品信息</a>
															</span>
														</div>
													</div>
													<div class="body">
